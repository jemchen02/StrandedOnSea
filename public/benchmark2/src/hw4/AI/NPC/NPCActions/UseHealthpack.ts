...
export default class UseHealthpack extends NPCAction {
    ...
    public performAction(target: Battler): void {
        const healthPackItem = this.actor.inventory.find(item => item.constructor === Healthpack);
        if (healthPackItem) {
            this.actor.inventory.remove(healthPackItem.id);
            target.health += (<Healthpack>healthPackItem).health;
